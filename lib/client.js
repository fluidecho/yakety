...
const preview       = require('preview')('yakkety-client');
...
var YakketyClient = function () {
	preview('YakketyClient');
	...
	this.options = {
		...
		headers: {
			'yakkety': '0.0.1',
			Codec: 'nws+smp-super-0+yakkety',		// Naked Websocket + Streaming message Protocol - Supper (Protocol) - Version (ident) + yakkety.
			'Codec-Encoding': 'utf8'
		},
		ident: 0,		// SMP.Super.Ident = 0:15 Protocol Version Number
		schemaURL: 'https://git.io/v1yU0'		// SMP.Super.Schema (https://github.com/fluidecho/yakkety/tree/v0.0.1)
	};  
	...
	this.message = function (msg, method) {
		...
		var binmsg = smp.super( new Buffer(JSON.stringify(meta)), new Buffer(msg), new Buffer(this.options.schemaURL), this.options.ident );	
	...
	this.request = function (method, req, rep) {
		...
		var binmsg = smp.super( new Buffer(JSON.stringify(meta)), new Buffer(req), new Buffer(this.options.schemaURL), this.options.ident );		// arg[0] = id, arg[1] = payload message data
		preview('binmsg', binmsg);
...
util.inherits(YakketyClient, events.EventEmitter); 
module.exports = YakketyClient;
...
YakketyClient.prototype._connect = function (fn) {
